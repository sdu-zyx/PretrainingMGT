...
def _get_model(args: AttrDict) -> nn.Module:
    ...
    if args.item_init_emb_path is not None:
        ...
        model.embed_item_GMF.weight.data.copy_(torch.from_numpy(item_init_emb))
        model.embed_item_GMF.requires_grad_(not args.freeze_item_init_emb)
    return model
